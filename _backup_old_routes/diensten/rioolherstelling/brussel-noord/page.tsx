...
export default function Page() {
  const municipalities = REGION_CITIES["brussel-noord"] ?? [];
  const muniText = municipalities.slice(0, 12).join(", ");

  const intro =
    "Lekkage, verzakking of defecte riolering in Brussel Noord? Turbo Services voert snelle en professionele rioolherstellingen uit." +
    (muniText ? `\n\nWerkgebied: ${muniText} en omgeving.` : "");

  const sections = [
  {
    "title": "Noodherstellingen",
    "body": "Lekkage, verzakking of defecte riolering in Brussel Noord? Turbo Services voert snelle en professionele rioolherstellingen uit."
  },
  {
    "title": "Wat we doen",
  ...
  const ctaBody =
    "Neem contact op voor een snelle diagnose en herstelling in Brussel Noord." +
    (muniText ? `\n\nWerkgebied: ${muniText} en omgeving.` : "");
