...
export const metadata: Metadata = {
  title: "Camera-inspectie in Antwerpen Zuidrand",
...
export default function Page() {
  const municipalities = REGION_CITIES["antwerpen-zuidrand"] ?? [];
  const muniText = municipalities.slice(0, 12).join(", ");

  const intro =
    "Nood aan een professionele camera-inspectie in Antwerpen Zuidrand? Wij brengen exact in beeld waar het probleem zit, zonder hak- of breekwerk." +
    (muniText ? `\n\nWerkgebied: ${muniText} en omgeving.` : "");

  const sections = [
  {
    "title": "Camera-inspectie",
    "body": "Nood aan een professionele camera-inspectie in Antwerpen Zuidrand? Wij brengen exact in beeld waar het probleem zit, zonder hak- of breekwerk."
  },
  {
    "title": "Wat we doen",
  ...
  const ctaBody =
    "Wij brengen uw riolering in Antwerpen Zuidrand exact in beeld en geven een duidelijk advies voor de juiste oplossing." +
    (muniText ? `\n\nWerkgebied: ${muniText} en omgeving.` : "");
