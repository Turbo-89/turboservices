...
export const metadata: Metadata = {
  title: "Ontstoppingen in Noordrand Brussel",
...
export default function Page() {
  const municipalities = REGION_CITIES["noordrand-brussel"] ?? [];
  const muniText = municipalities.slice(0, 12).join(", ");

  const service = getServiceByKey("ontstoppingen");

  const intro =
    "Verstopte wc, lavabo, douche of hoofdriolering in Noordrand Brussel? Turbo Services lost het op met professionele machines, ook â€™s avonds en in het weekend aan hetzelfde tarief." +
    (muniText ? `\n\nWerkgebied: ${muniText} en omgeving.` : "");

  const sections = [
  {
    "title": "Ontstoppingen",
    "body": "Verstopte wc, lavabo, douche of hoofdriolering in Noordrand Brussel? Turbo Services lost het op met professionele machines, ook â€™s avonds en in het weekend aan hetzelfde tarief."
  },
  {
    "title": "Wat we doen",
  ...
  },
  {
    ...
    if (idx === 0) {
      return {
        ...s,
        body: s.body + `\n\nActief in Noordrand Brussel: ${muniText} en omgeving.`
      };
    }
    return s;
  });
  ...
  const ctaBody =
    "Beschrijf kort het probleem en voeg indien mogelijk een foto toe. Wij koppelen snel terug met een concreet tijdsblok in Noordrand Brussel." +
    (muniText ? `\n\nWerkgebied: ${muniText} en omgeving.` : "");
