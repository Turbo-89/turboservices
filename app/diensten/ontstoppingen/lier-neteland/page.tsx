...
export const metadata: Metadata = {
  title: "Ontstoppingen in Lier & Neteland",
...
export default function Page() {
  const municipalities = REGION_CITIES["lier-neteland"] ?? [];
  const muniText = municipalities.slice(0, 12).join(", ");

  const service = getServiceByKey("ontstoppingen");

  const intro =
    "Verstopte wc, lavabo, douche of hoofdriolering in Lier & Neteland? Turbo Services lost het op met professionele machines, ook â€™s avonds en in het weekend aan hetzelfde tarief." +
    (muniText ? `\n\nWerkgebied: ${muniText} en omgeving.` : "");

  const sections = [
  {
    "title": "Ontstoppingen",
    "body": "Verstopte wc, lavabo, douche of hoofdriolering in Lier & Neteland? Turbo Services lost het op met professionele machines, ook â€™s avonds en in het weekend aan hetzelfde tarief."
  },
  {
    "title": "Wat we doen",
  ...
  },
  {
    ...
    if (idx === 0) {
      return {
        ...s,
        body: s.body + `\n\nActief in Lier & Neteland: ${muniText} en omgeving.`
      };
    }
    return s;
  });
  ...
  const ctaBody =
    "Beschrijf kort het probleem en voeg indien mogelijk een foto toe. Wij koppelen snel terug met een concreet tijdsblok in Lier & Neteland." +
    (muniText ? `\n\nWerkgebied: ${muniText} en omgeving.` : "");
