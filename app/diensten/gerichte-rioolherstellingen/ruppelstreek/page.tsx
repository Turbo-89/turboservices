...
export const metadata: Metadata = {
  title: "Gerichte rioolherstellingen in Ruppelstreek | Turbo Services",
  description:
    "Gerichte rioolherstellingen aan afvoer en riolering in Ruppelstreek, inclusief breuken, verzakkingen en lokale schade. Diagnose vooraf en doelgerichte herstelling.",
};

export default function Page() {
  const regionKey = "ruppelstreek" as const;
  const regionLabel = "Ruppelstreek";
  const municipalities = REGION_CITIES[regionKey] ?? [];
  const muniText = municipalities.slice(0, 12).join(", ");

  const intro =
    "Lekkage, verzakking of lokale schade aan de riolering in Ruppelstreek? Turbo Services voert gerichte rioolherstellingen uit met diagnose vooraf en een doelgerichte aanpak." +
  ...
  const ctaBody =
    "Neem contact op voor een snelle diagnose en gerichte rioolherstelling in Ruppelstreek." +
    (muniText ? `\n\nWerkgebied: ${muniText} en omgeving.` : "");
