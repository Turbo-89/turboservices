...
export const metadata: Metadata = {
  ...
  description:
    "Gerichte rioolherstellingen aan afvoer en riolering in Leuven & Dijleland, inclusief breuken, verzakkingen en lokale schade. Diagnose vooraf en doelgerichte herstelling.",
};

export default function Page() {
  const regionKey = "leuven-dijleland" as const;
  const regionLabel = "Leuven & Dijleland";
  const municipalities = REGION_CITIES[regionKey] ?? [];
  const muniText = municipalities.slice(0, 12).join(", ");

  const intro =
    "Lekkage, verzakking of lokale schade aan de riolering in Leuven & Dijleland? Turbo Services voert gerichte rioolherstellingen uit met diagnose vooraf en een doelgerichte aanpak." +
  ...
  const sections = [
    ...
    if (idx === 0) {
      return {
        ...s,
        body: s.body + `\n\nActief in Leuven & Dijleland: ${muniText} en omgeving.`,
      };
    }
    return s;
  });
  ...
  const ctaBody =
    "Neem contact op voor een snelle diagnose en gerichte rioolherstelling in Leuven & Dijleland." +
    (muniText ? `\n\nWerkgebied: ${muniText} en omgeving.` : "");
