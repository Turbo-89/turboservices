...
export const metadata: Metadata = {
  ...
  description:
    "Gerichte rioolherstellingen aan afvoer en riolering in Antwerpen Stad, inclusief breuken, verzakkingen en lokale schade. Diagnose vooraf en doelgerichte herstelling.",
};

export default function Page() {
  const regionKey = "antwerpen-stad" as const;
  const regionLabel = "Antwerpen Stad";
  const municipalities = REGION_CITIES[regionKey] ?? [];
  const muniText = municipalities.slice(0, 12).join(", ");

  const intro =
    "Lekkage, verzakking of lokale schade aan de riolering in Antwerpen Stad? Turbo Services voert gerichte rioolherstellingen uit met diagnose vooraf en een doelgerichte aanpak." +
  ...
  const ctaBody =
    "Neem contact op voor een snelle diagnose en gerichte rioolherstelling in Antwerpen Stad." +
    (muniText ? `\n\nWerkgebied: ${muniText} en omgeving.` : "");
