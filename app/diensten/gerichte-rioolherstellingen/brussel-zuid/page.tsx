...
export const metadata: Metadata = {
  ...
  description:
    "Gerichte rioolherstellingen aan afvoer en riolering in Brussel Zuid, inclusief breuken, verzakkingen en lokale schade. Diagnose vooraf en doelgerichte herstelling.",
};

export default function Page() {
  const regionKey = "brussel-zuid" as const;
  const regionLabel = "Brussel Zuid";
  const municipalities = REGION_CITIES[regionKey] ?? [];
  const muniText = municipalities.slice(0, 12).join(", ");

  const intro =
    "Lekkage, verzakking of lokale schade aan de riolering in Brussel Zuid? Turbo Services voert gerichte rioolherstellingen uit met diagnose vooraf en een doelgerichte aanpak." +
  ...
  const ctaBody =
    "Neem contact op voor een snelle diagnose en gerichte rioolherstelling in Brussel Zuid." +
    (muniText ? `\n\nWerkgebied: ${muniText} en omgeving.` : "");
