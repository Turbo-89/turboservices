...
export const metadata: Metadata = {
  ...
  description:
    "Gerichte rioolherstellingen aan afvoer en riolering in Brussel Centrum, inclusief breuken, verzakkingen en lokale schade. Diagnose vooraf en doelgerichte herstelling.",
};

export default function Page() {
  const regionKey = "brussel-centrum" as const;
  const regionLabel = "Brussel Centrum";
  const municipalities = REGION_CITIES[regionKey] ?? [];
  const muniText = municipalities.slice(0, 12).join(", ");

  const intro =
    "Lekkage, verzakking of lokale schade aan de riolering in Brussel Centrum? Turbo Services voert gerichte rioolherstellingen uit met diagnose vooraf en een doelgerichte aanpak." +
  ...
  const ctaBody =
    "Neem contact op voor een snelle diagnose en gerichte rioolherstelling in Brussel Centrum." +
    (muniText ? `\n\nWerkgebied: ${muniText} en omgeving.` : "");
