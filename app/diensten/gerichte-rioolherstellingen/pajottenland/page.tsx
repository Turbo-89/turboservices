...
export const metadata: Metadata = {
  ...
  description:
    "Gerichte rioolherstellingen aan afvoer en riolering in Pajottenland, inclusief breuken, verzakkingen en lokale schade. Diagnose vooraf en doelgerichte herstelling.",
};

export default function Page() {
  const regionKey = "pajottenland" as const;
  const regionLabel = "Pajottenland";
  const municipalities = REGION_CITIES[regionKey] ?? [];
  const muniText = municipalities.slice(0, 12).join(", ");

  const intro =
    "Lekkage, verzakking of lokale schade aan de riolering in Pajottenland? Turbo Services voert gerichte rioolherstellingen uit met diagnose vooraf en een doelgerichte aanpak." +
  ...
  const ctaBody =
    "Neem contact op voor een snelle diagnose en gerichte rioolherstelling in Pajottenland." +
    (muniText ? `\n\nWerkgebied: ${muniText} en omgeving.` : "");
