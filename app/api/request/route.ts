...
const TO_EMAIL       = (process.env.CONTACT_EMAIL || 'turbobv89@gmail.com').trim();
// In sandbox mag je enkel naar je account-mail sturen. Zet dit op jouw Resend account e-mail.
const RESEND_TEST_TO = (process.env.RESEND_TEST_TO || 'turbobv89@gmail.com').trim();
// In productie (na domeinverificatie) kan je MAIL_FROM zetten op jouw domein
const FROM           = (process.env.MAIL_FROM || 'Turbo Services <onboarding@resend.dev>').trim();
// Feature toggle: klantmails volledig aan/uit
...
export async function POST(req: Request) {
  try {
    ...
    // ---- Klantbevestiging (sandbox-veilig) ----
    // - Alleen sturen als:
    //   a) SEND_CUSTOMER_EMAILS = true, én
    //   b) we niet in sandbox zitten (dus domein verified), of email === RESEND_TEST_TO
    let klantMailVerzonden = false;
    if (SEND_CUSTOMER && email && email.trim()) {
      const lowered = email.trim().toLowerCase();
      const inSandbox = FROM.endsWith('@resend.dev>') || FROM.includes('resend.dev');
      const magInSandbox = lowered === RESEND_TEST_TO.toLowerCase();

      if (!inSandbox || magInSandbox) {
        const klantHtml = `
        ...
        `;
        await sendEmail(lowered, 'Aanvraag ontvangen – Turbo Services', klantHtml, TO_EMAIL);
        klantMailVerzonden = true;
      }
    }
